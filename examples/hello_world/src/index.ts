...
function selectRubyFiles(manifest: ManifestShape): string[] {
  ...
  const hibanaRootPath = 'lib/hibana.rb';
  const hibanaDslPath = 'lib/hibana/dsl.rb';

  if (entrypoint) {
    unique.delete(entrypoint);
  }

  const hasHibanaRoot = unique.delete(hibanaRootPath);
  const hasHibanaDsl = unique.delete(hibanaDslPath);

  ordered.push(...Array.from(unique).sort((a, b) => a.localeCompare(b)));

  if (hasHibanaDsl) {
    ordered.push(hibanaDslPath);
  }

  if (hasHibanaRoot) {
    ordered.push(hibanaRootPath);
  }
