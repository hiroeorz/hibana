...
- **hibana deploy**
  - `bundle exec rake wasm:build` と `npm run build` を順に実行し、生成物を `wrangler deploy` へ渡して本番環境にデプロイする。
  - `--skip-ruby-build` / `--skip-assets-build` で前処理を省略でき、残りの引数は wrangler へそのまま透過する。
...
これにより、明示的でテストしやすく、拡張性の高い設計を実現する。初期実装では `require 'hibana'` が DSL を読み込み、登録されたルートは `Hibana.app` に集約されて `Hibana::ENTRYPOINT` として WASM ランタイムに公開される。
